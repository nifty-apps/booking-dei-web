...
const EmployeeDetails = () => {
  ...
  } = useQuery(GET_TRANSACTION_BY_FILTER, {
    variables: {
      transactionFilter: {
        contact: userid,
      },
  ...
  const filteredEmployeeList = employeeData?.transactionByFilter.filter(
    (employeeInformation) => {
      const lowercaseSearchText = searchText.toLowerCase();
      const formattedDate = new Date(employeeInformation?.date).toLocaleString(
        undefined,
        {
          year: "numeric",
          month: "2-digit",
      ...
      return (
        employeeInformation?.contact.name
          ?.toLowerCase()
          .includes(lowercaseSearchText) ||
        formattedDate.includes(lowercaseSearchText) ||
        employeeInformation?.category
          ?.toLowerCase()
          .includes(lowercaseSearchText) ||
        employeeInformation?.subCategory
          ?.toLowerCase()
          .includes(lowercaseSearchText) ||
        employeeInformation?.method
          ?.toLowerCase()
          .includes(lowercaseSearchText) ||
        employeeInformation?.amount
          ?.toString()
          .toLowerCase()
          .includes(lowercaseSearchText)
      );
  ...
  return (
    <>
      ...
      <div className="py-5 px-2 flex">
        ...
        <div className="pl-2">
          <span className="text-lg block font-semibold text-gray-700">
            {contactData?.contacts[0].name}
          </span>
          <span className="block text-sm font-semibold text-gray-600">
            {contactData?.contacts[0].phone}
          </span>
        </div>
      </div>
